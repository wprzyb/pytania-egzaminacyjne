...
function generateQuestions(data, olddata, category, module) {

    for (var i = 0; i < data.length; i++) {

        let title = `Sugestia dotycząca pytania kat. ${category}, dział ${module}, nr ${data[i].id}`

        let issue = `
**Aktualna treść pytania:** ${data[i].pytanie}

**Odpowiedzi:** ${data[i].odpa}, ${data[i].odpb}, ${data[i].odpc}
...
`
        ...
        if (questionData.zmienione !== "") {
            console.log("QID: "+questionData.id)
            console.log("QZM: "+questionData.zmienione)
            console.log("QOID: "+olddata[parseInt(questionData.zmienione)].id)
            console.log("-----")
            entry += '<div class="card"><div class="card-content"><div class="content">' + '<p>' + htmldiff( olddata[parseInt(questionData.zmienione) -1 ].pytanie, questionData.pytanie )  + '</p><p>' + htmldiff( olddata[parseInt(questionData.zmienione) -1 ].odpa, questionData.odpa )  + '</p><p>' + htmldiff( olddata[parseInt(questionData.zmienione) -1 ].odpb, questionData.odpb )  + '</p><p>' + htmldiff( olddata[parseInt(questionData.zmienione) -1 ].odpc, questionData.odpc )  + '</p></div></div></div><hr>';
        }
